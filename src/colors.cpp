...
std::wstring BuildOscSet(Rgb c) {
    ...
    std::wstring out(L"\x1b]11;rgb:");
    out.reserve(22);
    AppendHexByteLower(out, static_cast<unsigned int>(c.r));
    out.push_back(L'/');
    AppendHexByteLower(out, static_cast<unsigned int>(c.g));
    out.push_back(L'/');
    AppendHexByteLower(out, static_cast<unsigned int>(c.b));
    out.append(L"\x1b\\");
    return out;
}
...
std::wstring BuildSgrBg(Rgb c) {
    ...
    std::wstring out(L"\x1b[48;2;");
    out.reserve(24);
    AppendUnsignedDecimal(out, static_cast<unsigned int>(c.r));
    out.push_back(L';');
    AppendUnsignedDecimal(out, static_cast<unsigned int>(c.g));
    out.push_back(L';');
    AppendUnsignedDecimal(out, static_cast<unsigned int>(c.b));
    out.push_back(L'm');
    return out;
}
