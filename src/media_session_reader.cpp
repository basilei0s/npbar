...
using SessionCommand =
    HRESULT (STDMETHODCALLTYPE MC::IGlobalSystemMediaTransportControlsSession::*)(void**);
...
std::wstring HResultMessage(HRESULT hr) {
    ...
    AppendHex32Upper(out, static_cast<unsigned long>(hr));
    return out;
}
...
PlaybackStatus PlaybackStatusFromGSMTC(MC::GlobalSystemMediaTransportControlsSessionPlaybackStatus s) {
    using S = MC::GlobalSystemMediaTransportControlsSessionPlaybackStatus;
    switch (s) {
        case S::GlobalSystemMediaTransportControlsSessionPlaybackStatus_Playing:
            return PlaybackStatus::Playing;
        case S::GlobalSystemMediaTransportControlsSessionPlaybackStatus_Paused:
            return PlaybackStatus::Paused;
        case S::GlobalSystemMediaTransportControlsSessionPlaybackStatus_Stopped:
            return PlaybackStatus::Stopped;
        case S::GlobalSystemMediaTransportControlsSessionPlaybackStatus_Changing:
...
public:
    ...
    void WaitForIdle() const {
        while (InterlockedCompareExchange(
                   const_cast<volatile LONG*>(&activeCallbacks_),
                   0,
                   0) > 0) {
            Sleep(1);
        }
...
struct MediaSessionReader::Impl {
    ...
    PlaybackStatus                    cachedStatus = PlaybackStatus::Unknown;
    ...
    std::mutex                        bindMutex;
    std::mutex                        fallbackPollMutex;
    std::chrono::steady_clock::time_point lastFallbackPoll{};
    std::atomic<bool>                 managerEventFallback{false};
    std::atomic<bool>                 sessionEventFallback{false};
    ...
    bool SubmitCommand(SessionCommand command);
};
...
void MediaSessionReader::Impl::RefreshPlaybackInfo(
    ...
    if (FAILED(s->GetPlaybackInfo(pb.ReleaseAndGetAddressOf())) || !pb) {
        {
            std::lock_guard<std::mutex> lk(cacheMutex);
            cachedStatus = PlaybackStatus::Unknown;
        }
        SignalUpdate();
        return;
    }
    ...
    if (SUCCEEDED(pb->get_PlaybackStatus(&st))) status = PlaybackStatusFromGSMTC(st);
    {
        std::lock_guard<std::mutex> lk(cacheMutex);
        cachedStatus = status;
    }
...
void MediaSessionReader::Impl::RebindCurrentSession() {
    ...
    session.Reset();
    sessionEventFallback.store(false, std::memory_order_release);
    ...
    // session pointers.
    {
        ...
        else { sessionEventFallback.store(true, std::memory_order_release); }
    }
    {
        auto* h = new EventHandler(
...
void MediaSessionReader::Impl::PollFallbackIfNeeded() {
    if (!managerEventFallback.load(std::memory_order_acquire)
        && !sessionEventFallback.load(std::memory_order_acquire)) {
        return;
    }

    using clock = std::chrono::steady_clock;
    constexpr auto kFallbackPollInterval = std::chrono::milliseconds{1000};
    const auto now = clock::now();
    {
        std::lock_guard<std::mutex> lk(fallbackPollMutex);
        if (lastFallbackPoll.time_since_epoch().count() != 0
    ...
    ComPtr<MC::IGlobalSystemMediaTransportControlsSession> current;
    {
        std::lock_guard<std::mutex> lk(bindMutex);
        current = session;
    }
    if (!current) return;

    RefreshAumid(current.Get());
    RefreshMediaProperties(current.Get());
    RefreshTimeline(current.Get());
    RefreshPlaybackInfo(current.Get());
}
...
MediaSessionReader::MediaSessionReader() {
    ...
    hr = HString::Create(
        ...
        static_cast<UINT32>(
            (sizeof(kRC_GlobalSystemMediaTransportControlsSessionManager) / sizeof(wchar_t)) - 1),
    ...
    // Subscribe to manager-level events.
    {
        ...
        else { impl_->managerEventFallback.store(true, std::memory_order_release); }
    }
    {
...
std::optional<NowPlayingInfo> MediaSessionReader::GetCurrent() {
    ...
    if (rawPosition && lastUpdatedUtcTicks > 0 && info.status == PlaybackStatus::Playing) {
...
// Transport commands. Each grabs a strong reference to the current session
// under the bind mutex (so a concurrent CurrentSessionChanged event can't
// reset it while we're calling), then issues the async command on the
// local copy. We don't wait for the result — the player handles it on its
// own time and any visible effect will arrive via PlaybackInfoChanged.

bool MediaSessionReader::Impl::SubmitCommand(SessionCommand command) {
    ComPtr<MC::IGlobalSystemMediaTransportControlsSession> s;
    {
        std::lock_guard<std::mutex> lk(bindMutex);
        s = session;
    }
    if (!s) return false;
    void* op = nullptr;
    HRESULT hr = (s.Get()->*command)(&op);
    if (op) static_cast<IUnknown*>(op)->Release();
    return SUCCEEDED(hr);
}

bool MediaSessionReader::TogglePlayPause() {
    if (!ready_ || !impl_) return false;
    return impl_->SubmitCommand(
        &MC::IGlobalSystemMediaTransportControlsSession::TryTogglePlayPauseAsync);
}

bool MediaSessionReader::SkipNext() {
    if (!ready_ || !impl_) return false;
    return impl_->SubmitCommand(&MC::IGlobalSystemMediaTransportControlsSession::TrySkipNextAsync);
}

bool MediaSessionReader::SkipPrevious() {
    if (!ready_ || !impl_) return false;
    return impl_->SubmitCommand(
        &MC::IGlobalSystemMediaTransportControlsSession::TrySkipPreviousAsync);
}
