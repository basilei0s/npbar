...
int RunOneShot(MediaSessionReader& reader, const CliOptions& opt) {
    ...
    line.reserve(512);
    if (!info) {
        if (RenderEmpty(ctx, opt.emptyMode, line)) WriteUtf8Line(line);
        return 0;
    }

    RenderBar(*info, ctx, line);
    WriteUtf8Line(line);
    return 0;
}
...
int RunWatch(MediaSessionReader& reader, const CliOptions& opt) {
    ...
    // Help overlay state: shown while the user holds '?'.
    bool helpHeld    = false;
    ...
    while (!g_stopFlag.load(std::memory_order_relaxed)) {
        ...
        std::optional<NowPlayingInfo> info;
        if (showingHelp) {
            RenderHelp(ctx, line);
        } else {
            info = reader.GetCurrent();
            if (info) {
                RenderBar(*info, ctx, line, &scroll);
            } else {
                if (!RenderEmpty(ctx, opt.emptyMode, line)) line.clear();
            }
        }
        ...
        while (remaining > 0 && !g_stopFlag.load(std::memory_order_relaxed)) {
            ...
            switch (WaitForKeyOrUpdate(upd, step, helpHeld)) {
                ...
                case WaitOutcome::Updated:
                    remaining = 0;
                    break;
                case WaitOutcome::TogglePlay:
                    reader.TogglePlayPause();
                    remaining = 0;
                    break;
                case WaitOutcome::SkipNext:
                    reader.SkipNext();
                    remaining = 0;
                    break;
                case WaitOutcome::SkipPrev:
                    reader.SkipPrevious();
                    remaining = 0;
                    break;
            ...
            // If '?' pressed/released this turn, re-render immediately to
            // show/hide the overlay.
            if (helpHeld != showingHelp) {
                showingHelp = helpHeld;
                remaining = 0;
            }
        }
    }
