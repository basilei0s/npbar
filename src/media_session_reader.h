...
public:
    ...
    // Transport control commands forwarded to the current GSMTC session.
    // Each returns true on a successful submission (false if there is no
    // session or the API call failed). The actual play/pause/skip is
    // performed asynchronously by the source app — we fire-and-forget.
