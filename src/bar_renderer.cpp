...
std::wstring_view StatusText(PlaybackStatus status) {
    switch (status) {
...
std::wstring BuildTimeSegment(const NowPlayingInfo& info,
    ...
    std::wstring out;
    out.reserve(16);
    if (position) AppendDuration(out, *position);
    else          out.append(L"--:--");
    out.append(L" / ");
    AppendDuration(out, *info.duration);
    return out;
}
...
void AppendColorized(std::wstring& out, const Segment& seg, const Palette& pal,
    ...
    switch (seg.kind) {
        ...
        case Kind::Status: {
            ...
            if (info.status == PlaybackStatus::Playing)     color = pal.statusPlaying;
            else if (info.status == PlaybackStatus::Paused) color = pal.statusPaused;
            wrap(color, seg.text);
            return;
        }
        case Kind::Time: {
...
std::optional<std::chrono::milliseconds> EstimatePosition(const NowPlayingInfo& info) {
    ...
    if (info.status != PlaybackStatus::Playing) return info.position;
...
void RenderHelp(const RenderContext& ctx, std::wstring& out) {
    const auto& pal = ctx.palette;
    const std::wstring_view sep = SymSep(ctx.unicode);
    const std::wstring_view arrowL = ctx.unicode ? std::wstring_view(L"←") : std::wstring_view(L"<");
    const std::wstring_view arrowR = ctx.unicode ? std::wstring_view(L"→") : std::wstring_view(L">");

    auto appendWrapped = [&](std::wstring_view color, std::wstring_view text) {
        if (!color.empty() && pal.enabled) out.append(color);
        out.append(text);
        if (!color.empty() && pal.enabled) out.append(pal.reset);
    };

    auto appendSep = [&]() {
        appendWrapped(pal.separator, sep);
    };

    auto appendKeyWithArrow = [&](std::wstring_view prefix, std::wstring_view arrow) {
        if (!pal.artist.empty() && pal.enabled) out.append(pal.artist);
        out.append(prefix);
        out.append(arrow);
        if (!pal.artist.empty() && pal.enabled) out.append(pal.reset);
    };

    out.clear();
    out.reserve(160);

    // [?] Hotkeys │ space  play/pause │ n/→  next │ p/←  prev │ q/Esc  quit
    appendWrapped(pal.icon, L"?");
    out.push_back(L' ');
    appendWrapped(pal.source, L"Hotkeys");
    appendSep();

    appendWrapped(pal.artist, L"space");
    out.push_back(L' ');
    appendWrapped(pal.title, L"play/pause");
    appendSep();

    appendKeyWithArrow(L"n / ", arrowR);
    out.push_back(L' ');
    appendWrapped(pal.title, L"next");
    appendSep();

    appendKeyWithArrow(L"p / ", arrowL);
    out.push_back(L' ');
    appendWrapped(pal.title, L"prev");
    appendSep();

    appendWrapped(pal.artist, L"q / Esc");
    out.push_back(L' ');
    appendWrapped(pal.title, L"quit");

    // Hard truncate if the terminal is narrower than the full overlay.
    const size_t maxW = static_cast<size_t>(ctx.width > 0 ? ctx.width : 0);
    if (maxW > 0 && DisplayWidth(out) > maxW) {
        out = TruncateToWidth(out, maxW, ctx.unicode ? L"…" : L"...");
    }
}

void RenderPlain(const NowPlayingInfo& info, std::wstring& s) {
    std::wstring_view artist = info.artist.empty() ? std::wstring_view(L"Unknown artist") : std::wstring_view(info.artist);
    std::wstring_view title  = info.title.empty()  ? std::wstring_view(L"Unknown title")  : std::wstring_view(info.title);
    s.clear();
    ...
    if (info.duration) {
        ...
        s.append(L" [");
        if (pos) AppendDuration(s, *pos); else s.append(L"--:--");
        s.append(L" / ");
        AppendDuration(s, *info.duration);
        s.push_back(L']');
    }
}
...
void RenderBar(const NowPlayingInfo& info, const RenderContext& ctx, std::wstring& out,
    ...
    SetSeg(segs[3], Kind::Status, std::wstring(StatusText(info.status)));
    ...
    if (total > maxW && segs[4].active && info.duration) {
        std::wstring durationOnly;
        durationOnly.reserve(8);
        AppendDuration(durationOnly, *info.duration);
        SetSeg(segs[4], Kind::Time, std::move(durationOnly));
        total = TotalWidth(segs, sepW);
    }
